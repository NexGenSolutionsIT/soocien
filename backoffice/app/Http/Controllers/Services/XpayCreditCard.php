...
class Xpay extends Controller
{
    ...
    public function createTransactionCredit(Request $request): mixed
    {
        ...
        $data = [
            ...
            'card_number' => $validateData['card']['number'],
        ];

        $responseTokenizeCard = $this->tokenizeCard($data);
        if ($responseTokenizeCard['status'] == 200) {
            $data = [
                'access_token' => $accessToken,
                'amount' => intval($validateData['value'] * 100),
                'installments' => $validateData['installments'],
                'cardHolderName' => $validateData['card']['name'],
                'number_token' => $responseTokenizeCard['data']['number_token'],
                'cvv' => $validateData['card']['cvv'],
                'expirityDate' => $validateData['card']['expirationMonth'] . str_replace('20', '', $validateData['card']['expirationYear']),
                'soft_descriptor' => $validateData['soft_descriptor'],
                'customerFirstName' => $validateData['payerName'],
                'customerLastName' => $validateData['payerName'],
                'customerEmail' => $validateData['payerEmail'],
                'customerPhone' => '5511987654321',
                "customerAddress" => "Rua Teste, 123",
                "customerCity" => "Sao Paulo",
                "customerState" => "SP",
                "customerZipCode" => "0645404"
            ];
            ...
            $responseChargerPaymentAndIntentionPayment = [
                ...
                'intentionPayment' => $responseTokenizeCard
            ];
            ...
            $this->createFinanceMovement($orderCredit);

            return response()->json($data, $statusCode);
        }
        return response()->json($responseTokenizeCard, 422);
    }
     ...
     */
    public function cancelChargePayment(Request $request): string
    {
        if ($request->header('X-API-SECRET') !== $this->apiSecreteKey) {
            return response()->json(['error' => 'Unauthorized'], 401);
        }
     ...
     */
    public function showTransactions(Request $request): string
    {
        if ($request->header('X-API-SECRET') !== $this->apiSecreteKey) {
            return response()->json(['error' => 'Unauthorized'], 401);
        }
    ...
    public function tokenizeCard($data): array
    {
        $data = array_merge(
        ...
        $result = Http::withHeaders([
            'authorizationToken' => $this->authorization,
            'Content-Type' => 'application/json'
        ])->post($this->url . 'creditcard-payment/tokenize', $data);
    ...
    /**
     * createFinanceMovement function
     *
     * @param array $data
     * @return void
     */
    private function createFinanceMovement($data): void
    {

        $client = ClientModel::where('id', $data['client_id'])->first();
        $affiliate = ClientModel::where('id', $client->indicator_id)->first();
        $admin = AdminModel::find(1);

        $adminBalance = ($data['amount'] * 20) / 100;
        $admin->balance += $adminBalance;
        $admin->save();

        if ($client->indicator_id) {
            $affiliateBalance = (($data['amount'] - $adminBalance) * 5) / 100;
            $affiliate->balance += $affiliateBalance;
            $affiliate->save();

            $userBalance = $data['amount'] - ($adminBalance + $affiliateBalance);
        } else {
            $userBalance = $data['amount'] - $adminBalance;
        }
